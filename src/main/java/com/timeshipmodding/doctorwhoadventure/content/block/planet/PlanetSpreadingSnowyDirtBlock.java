...
public abstract class PlanetSpreadingSnowyDirtBlock extends PlanetSnowyDirtBlock {
    public String planet;

    protected PlanetSpreadingSnowyDirtBlock(String planet, BlockBehaviour.Properties properties) {
        super(properties);
        this.planet = planet;
    }

    public Block getDirtBlock(String planet) {
    ...
    protected void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
        if (!canBeGrass(state, level, pos)) {
            ...
            level.setBlockAndUpdate(pos, getDirtBlock(planet).defaultBlockState());
        } else {
            ...
            if (level.getMaxLocalRawBrightness(pos.above()) >= 9) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (level.getBlockState(blockpos).is(getDirtBlock(planet)) && canPropagate(blockstate, level, blockpos)) {
